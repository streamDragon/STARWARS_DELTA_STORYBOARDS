...
async function metrics(page){return page.locator('#stage').evaluate(el=>{
...
async function shot(page,t){
  ...
  await page.locator('#scrub').evaluate((el,v)=>{el.value=String(v);el.dispatchEvent(new Event('input',{bubbles:true}))},value);
  await page.waitForTimeout(900);
  const clockText=await page.locator('#time').textContent();const m=await metrics(page);const label=t.toFixed(1).padStart(4,'0');
  const stage=`frames/frame-${label}s.png`,full=`full-page/full-${label}s.png`;
  await page.locator('#stage').screenshot({path:path.join(OUT,stage)});await page.screenshot({path:path.join(OUT,full),fullPage:true});
...
try{
  ...
  report.loadedBanner=await page.locator('#drop').textContent();report.checks.movieLoaded=/GOLDEN/i.test(report.loadedBanner||'');report.checks.timelineHasEightBeats=await page.locator('#timeline .shot').count()===8;
  mark('play-smoke');const before=parseClock(await page.locator('#time').textContent())?.elapsed||0;await page.getByRole('button',{name:'PLAY',exact:true}).click();await page.waitForTimeout(2600);const after=parseClock(await page.locator('#time').textContent())?.elapsed||0;const playMetrics=await metrics(page);report.playSmoke={before,after,metrics:playMetrics,health:(await page.locator('.playHealth').textContent().catch(()=>''))||''};report.checks.playAdvanced=after-before>1.5;report.checks.playFrameVisible=playMetrics.visibleCanvasCount>0&&playMetrics.sampledNonDark>0;report.checks.playHealthOk=/PLAYBACK OK/i.test(report.playSmoke.health);
  const pause=page.getByRole('button',{name:'PAUSE',exact:true});if(await pause.count())await pause.click();
  mark('frame-scan');for(const t of TIMES)await shot(page,t);
  const diag=(await page.locator('#diag').textContent())||'';report.checks.noUnknownHandle=!/UNKNOWN_HANDLE/i.test(diag);report.checks.noPageErrors=report.pageErrors.length===0;report.checks.framesVisible=report.screenshots.every(s=>s.metrics.visibleCanvasCount>0&&s.metrics.sampledNonDark>0);
  ...
  mark('write-report');report.finishedAt=new Date().toISOString();await fs.writeFile(path.join(OUT,'report.json'),JSON.stringify(report,null,2));await fs.writeFile(path.join(OUT,'REPORT.md'),['# Cutscene Film QA','',`**Result:** ${report.summary}`,`**Movie:** ${MOVIE}`,'','## Checks',...Object.entries(report.checks).map(([k,v])=>`- ${v?'PASS':'FAIL'}: ${k}`),'',`Console errors: ${report.consoleErrors.length}`,`Page errors: ${report.pageErrors.length}`,`Network failures: ${report.networkFailures.length}`].join('\n'));await page.close().catch(()=>{});await context.close().catch(()=>{});await browser.close().catch(()=>{})
}
