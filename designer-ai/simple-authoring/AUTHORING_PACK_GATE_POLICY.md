...
- Every `type=fire` action authors an exact closed-world `projectileId`. The prepared Cutscene projectile vocabulary is exactly `CS_PROJECTILE_LASER`, `CS_PROJECTILE_AXE`, `CS_PROJECTILE_ROCKET` when the matching CURRENT exports the same list. `effectHandle`, `viaHandle`, gameplay projectile prefabs, filenames and fuzzy matching are never substitutes for projectile identity.
