...
## Cinematic quality contract: Unity-proven local upgrade V1

This upgraded context pack adds a directing overlay proven in Unity on 2026-08-25.
It narrows legal Simple V1 choices without changing schema or project identity.

Before authoring, read `05_CINEMATIC_QUALITY_START.md`,
`authoring/CINEMATIC_GOLD_SET_CURRENT.json`, and
`site-authoring/CINEMATIC_SHOT_GRAMMAR_CURRENT.json`.

Core quality invariants:

- one beat is one shot idea;
- one primary focal subject by default;
- prefer visually curated Gold handles;
- keep one coherent world-style family;
- `locationHandle` is the full-frame environment plate and is never a partial insert/effect;
- major portrait dialogue belongs in a dedicated Dialogue Stage/monitor cut;
- preserve screen direction and negative space for travel;
- use framing-appropriate actor scale;
- local VFX remain local;
- camera movement must be motivated;
- never encode engine workarounds into Simple V1 JSON.

The proven cinematic reference is
`authoring/CUTSCENE_SCRIPT_V1_CINEMATIC_REFERENCE_PROVEN_V2.json`.

The current local engine also treats Simple V1 location plates as zero-parallax full-frame plates and
performs a final full-frame cover refit after V3 camera materialization. Authoring should therefore state
clean cinematic intent rather than compensating with invented scale or duplicate background layers.
