(()=>{
  ...
  function simpleMotionFor(v,t,b){
    ...
    const state=norm(v.state),role=norm(v.role),acts=(b.actions||[]).filter(a=>a.subject===v.id||a.target===v.id),actText=norm(acts.map(a=>`${a.type} ${a.result||''}`).join(' '));
  ...
  function decorateSimpleObjects(w){
    ...
    for(const v of b.visible||[]){
      ...
      for(let i=0;i<n;i++){
        const c=canvases[ci++];if(!c)continue;const m=simpleMotionFor(v,t,b),f=clamp(num(v.screenWidthFraction,resolveHandle(v.handle)?.targetScreenFraction??.22),.025,1.5),o=spread(i,n);
  ...
  ensureHealth();
  console.info('[CUTSCENE_PREVIEW] Simple playback motion + PLAY health check ready');
})();